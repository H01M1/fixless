  ...
  return (
    <div className="min-h-screen">
      <header className="flex items-center justify-between px-5 pt-5 pb-2">
        ...
        <Link
          href="/add"
          className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold shadow-sm active:bg-indigo-700 transition-colors"
        >
          ...
          <div className="space-y-1.5">
            {urgentBillings.map(({ subscription, daysUntil }) => (
              <div key={subscription.id} className="flex items-center justify-between">
                <span className="text-xs text-rose-700 truncate max-w-[60%]">
                  {subscription.name}
                </span>
                <span className="text-xs text-rose-600 font-semibold whitespace-nowrap ml-2">
                  {formatDate(subscription.nextBillingDate)}（{formatDaysUntil(daysUntil)}）
